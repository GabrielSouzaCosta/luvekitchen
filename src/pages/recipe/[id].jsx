...
import { H2, H3, H4, H5, ListItem, P, TextMuted } from '../../styles/texts';
import { colors } from '../../styles/theme';
import { Rating } from '@mui/material';
import { IoMdGlobe, IoMdShare } from 'react-icons/io';
...
const Recipe = () => {
  ...
  useEffect(() => {
	if (recipe?.data.meals) {
		let ingredientsData = Object.keys(recipe?.data.meals[0])
			...
			<RecipeContainer>
				<FlexContainer>
					<div>
						...
						{/* Create new review */}
						{ctx?.userData?.accessToken && !ratings?.find((item) => item.user_id === ctx?.userData?.id) &&
							<ReviewItem
